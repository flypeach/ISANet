...
class BasicBlock(nn.Cell): # 迁移完成
    ...
    def __init__(self, inplanes, planes, stride = 1, downsample = None, groups = 1,
        ...
        self.conv1 = nn.Conv2d(in_channels=inplanes, out_channels=planes, kernel_size=3, stride=stride, 
                               padding=dilation, group=groups, has_bias=False, dilation=dilation)
...
class Bottleneck(nn.Cell): # 迁移完成
    ...
    def __init__(self, inplanes, planes, stride = 1, downsample = None,
        ...
        self.conv1 = nn.Conv2d(in_channels=inplanes, out_channels=width, kernel_size=1, stride=1, has_bias=False)
        self.bn1 = norm_layer(width)
        self.conv2 = nn.Conv2d(in_channels=width, out_channels=width, kernel_size=3, stride=stride, has_bias=False, pad_mode = 'pad', padding=dilation, dilation=dilation)
        self.bn2 = norm_layer(width)
        self.conv3 = nn.Conv2d(in_channels=width, out_channels=planes * self.expansion, kernel_size=1, stride=1, has_bias=False)
...
class ResNet(nn.Cell): # 迁移完成
    def __init__(
        ...
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=self.inplanes, kernel_size=7, stride=2, pad_mode = 'pad', padding=3, has_bias=False)
    ...
    def _make_layer(
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.SequentialCell(
                nn.Conv2d(in_channels=self.inplanes, out_channels=planes * block.expansion, kernel_size=1, stride=stride, has_bias=False),
